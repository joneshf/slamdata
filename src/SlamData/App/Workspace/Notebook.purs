...
--           (makeBlocks settingId active props.settings props.handle (deferred <<< modalVisibility) <$> notebooks)
--       ] ++ if vState.visible
--         then
--           [D.div
...
--              -> NotebookSpec
--              -> UI
--   makeBlocks settingId active config handle _ (NotebookSpec nb)
--     | settingId == nb.ident = D.div
--       [D.className $ "content" ++ maybeActive nb.ident active]
--       [settings {settings: config, handle: handle}]
